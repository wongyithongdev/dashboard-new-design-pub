...
const JOB_SERVICE_CATEGORIES: Record<string, string> = {
  ...
  "JO-2026-0009": "Plumbing",
  "JO-2026-0010": "IT Support",
};
...
const JOB_ORDER_DETAILS: Record<string, JobOrderDetail> = {
  "JO-2026-0001": {
    ...
    serviceCharge: 280.00, commision: 320, itemsTotal: 120.00,
  ...
  "JO-2026-0002": {
    ...
    serviceCharge: 0, commision: 180, itemsTotal: 0,
  ...
  "JO-2026-0003": {
    ...
    serviceCharge: 150.00, commision: 210, itemsTotal: 85.00,
  ...
  "JO-2026-0004": {
    ...
    serviceCharge: 120.00, commision: 150, itemsTotal: 30.00,
  ...
  "JO-2026-0006": {
    ...
    serviceCharge: 1800.00, commision: 480, itemsTotal: 620.00,
  ...
  "JO-2026-0007": {
    ...
    serviceCharge: 350.00, commision: 240, itemsTotal: 0,
  ...
  "JO-2026-0008": {
    ...
    serviceCharge: 320.00, commision: 175, itemsTotal: 210.00,
    photos: [], inprogressAt: "06 Jun 2026 11:00:00", completedAt: "06 Jun 2026 14:45:00", createdAt: "05 Jun 2026 09:30:00",
  },
  "JO-2026-0009": {
    jobNo: "JO-2026-0009", customerName: "Sunrise Residence", customerRef: "300-C003",
    phone: "019-882 7712", serviceDate: "05 Jun 2026 09:00:00", status: "Claimed",
    customerInfo: "NO 12, JALAN MATAHARI 3, TAMAN SUNRISE, 93350 KUCHING",
    customerReq: "Water heater leaking, unable to use",
    note: "Replaced pressure relief valve, resealed pipe joints", agentNote: "",
    agent: "Ahmad Farid", serviceType: "PLUMBING SERVICE",
    serviceItem: "Water Heater", serviceItemRef: "PL-012",
    serialNo: "WH-2023-0091", furtherDescription: "Rheem 40-gallon storage heater",
    serviceCharge: 180.00, commision: 310, itemsTotal: 95.00,
    photos: [], inprogressAt: "05 Jun 2026 09:00:00", completedAt: "05 Jun 2026 11:30:00", createdAt: "04 Jun 2026 14:00:00",
  },
  "JO-2026-0010": {
    jobNo: "JO-2026-0010", customerName: "TechCore Solutions Sdn Bhd", customerRef: "300-C002",
    phone: "082-233 456", serviceDate: "04 Jun 2026 10:00:00", status: "Settled",
    customerInfo: "LEVEL 8, WISMA TECHCORE, JALAN TUNKU ABDUL RAHMAN, 93100 KUCHING",
    customerReq: "Annual server maintenance and backup verification",
    note: "Full backup verified, firmware updated on all nodes", agentNote: "Renewed maintenance contract for 12 months",
    agent: "Wong Yi Thong", serviceType: "IT REMOTE SUPPORT (CONTRACT CUSTOMER)",
    serviceItem: "Server Maintenance", serviceItemRef: "IT-038",
    serialNo: "SRV-2022-0014", furtherDescription: "Dell PowerEdge R750, 3-node cluster",
    serviceCharge: 850.00, commision: 450, itemsTotal: 0,
    photos: [], inprogressAt: "04 Jun 2026 10:00:00", completedAt: "04 Jun 2026 16:00:00", createdAt: "01 Jun 2026 09:00:00",
  },
};
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-3 pt-3 pb-1.5">
        <button type="button" onClick={prevMonth} className="flex size-[26px] items-center justify-center rounded-[5px] text-[#9b9a97] transition-colors duration-75 hover:bg-[#f1f0ee] hover:text-[#37352f]">
          <ChevronLeft size={14} strokeWidth={2} />
        </button>
        <span className="text-[13px] font-semibold text-[#37352f]">{MONTH_NAMES[month]} {year}</span>
        <button type="button" onClick={nextMonth} className="flex size-[26px] items-center justify-center rounded-[5px] text-[#9b9a97] transition-colors duration-75 hover:bg-[#f1f0ee] hover:text-[#37352f]">
          ...
          return (
            <button
              ...
              className={[
                ...
                isSelected ? "bg-[#2383e2] font-semibold text-white"
                  : isToday ? "font-semibold text-[#2383e2] hover:bg-[#f1f0ee]"
      ...
      <div className="border-t border-[#e9e9e7] px-3 py-2 flex justify-end">
        <button type="button" onClick={onClose} className="rounded-[5px] bg-[#2383e2] px-3 py-1 text-[12px] font-medium text-white transition-colors duration-75 hover:bg-[#1a73d4]">
          Done
        </button>
      </div>
    </div>
...
function toInputTime(s: string) { return s.split(" ")[3]?.slice(0, 5) ?? "00:00"; }
...
/* ── Status meta — includes Claimed + Settled ── */
const STATUS_META: Record<string, { cls: string; dot: string }> = {
  ...
  "Claimed":     { cls: "bg-[#f0ebff] text-[#6d28d9]",  dot: "#6d28d9" },
  "Settled":     { cls: "bg-[#f5ede8] text-[#9f6b53]",  dot: "#9f6b53" },
  "Cancelled":   { cls: "bg-[#e3e2e0] text-[#787774]",  dot: "#9b9a97" },
};

export default function JobOrderManagementDetailPage() {
  const params = useParams();
  const taskId = typeof params.taskId === "string" ? params.taskId : "";
  const detail = JOB_ORDER_DETAILS[taskId] ?? null;
  ...
  void customerColor; void CustomerIcon;

  const serviceCategory = JOB_SERVICE_CATEGORIES[detail.jobNo] ?? "";
  const categoryEmoji: Record<string, string> = {
    "HVAC": "❄️", "IT Support": "💻", "Electrical": "⚡", "Plumbing": "🔧",
    "Cleaning": "✨", "Carpentry": "🪚", "Pest Control": "🐛",
    "Landscaping": "🌿", "Painting": "🎨", "Mechanical": "⚙️",
  };
  ...
  const chip = STATUS_META[form.status] ?? { cls: "bg-[#e3e2e0] text-[#787774]", dot: "#9b9a97" };
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-10 py-8 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <motion.div {...fu(0.04)}>
              ...
              <h1 className="text-[26px] font-bold tracking-[-0.4px] text-[#37352f] leading-[1.25]">{detail.customerName}</h1>
              <div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[13px] text-[#9b9a97]">
                ...
                {detail.phone && (
                  <>
                    ...
                    <a href={`tel:${detail.phone}`} className="transition-colors duration-75 hover:text-[#37352f]">{detail.phone}</a>
                  </>
                )}
              </div>
              {detail.customerInfo && (
                  ...
                  return (
                    <div key={step.label} className="flex gap-3">
                      ...
                      <div className={`flex flex-1 items-baseline justify-between gap-2 ${isLast ? "pb-0" : "pb-4"}`}>
                        <span className="text-[13px] font-medium" style={{ color: isDone ? step.dotColor : "#c7c7c7" }}>
                          {step.label}
                        </span>
          ...
          <div className="w-[440px] shrink-0 border-l border-[#e9e9e7] overflow-y-auto px-8 py-8 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                ...
                <AnimatePresence>
                  {openProp === "status" && (
                    <motion.div
                      ...
                      className="absolute left-0 top-7 z-30 w-[200px] origin-top-left rounded-[8px] border border-[#e9e9e7] bg-white p-1 shadow-[0_8px_24px_rgba(15,15,15,0.12)]"
                    >
                      {(["In Progress", "Pending", "Completed", "Claimed", "Settled", "Cancelled"] as const).map(s => {
                        const m = STATUS_META[s] ?? { cls: "bg-[#e3e2e0] text-[#787774]", dot: "#9b9a97" };
                        return (
                          <button
                            key={s}
                            type="button"
            ...
            <motion.div {...fu(0.14)}>
              <div className="flex flex-col -mx-2">
                ...
                >
                  ...
                  >
                    <span className="flex size-[18px] shrink-0 items-center justify-center rounded-full text-[9px] font-bold text-white" style={{ background: agentColor }}>
                      {agentInitial}
                    </span>
                    {form.agent}
                  </button>
            ...
            <motion.div {...fu(0.21)}>
              ...
              <div className="flex flex-col -mx-2">
                ...
                <div className="flex items-center min-h-[36px] rounded-[4px] px-2 transition-colors duration-75 hover:bg-[#f1f0ee]">
                  <span className="w-[150px] shrink-0 text-[14px] text-[#787774]">Commission</span>
                  {isEditing ? (
                    <div className="flex items-center gap-1 flex-1">
                      <span className="text-[13px] text-[#9b9a97]">RM</span>
                      <input
                        ...
                        onChange={e => set("commision", parseFloat(e.target.value) || 0)}
                        onFocus={e => e.target.select()}
                      />
                    </div>
                  ) : (
                    <span className="font-mono text-[14px] tabular-nums text-[#37352f]">RM {form.commision.toFixed(2)}</span>
                  )}
                </div>
