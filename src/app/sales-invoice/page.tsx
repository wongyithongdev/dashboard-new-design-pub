...
type StatusFilter = "all" | SalesInvoice["paymentStatus"];

const salesInvoices: SalesInvoice[] = [
  {
    invoiceNo: "SI-2026-0001",
    customer: "Vertex Retail Sdn Bhd",
    agent: "Wong Yi Thong",
    date: "08 Jun 2026",
    amount: "RM 1,640.00",
    paymentStatus: "Unpaid",
    currency: "MYR",
    outstandingAmount: "RM 1,640.00",
    details: [
      { description: "POS Terminal Setup", qty: 1, unitPrice: 900, amount: 900 },
      { description: "Barcode Scanner Bundle", qty: 2, unitPrice: 370, amount: 740 },
    ],
  },
  {
    ...
    currency: "MYR",
    outstandingAmount: "RM 428.15",
    details: [
      { description: "Inventory Sync Service", qty: 1, unitPrice: 420, amount: 420 },
      { description: "Receipt Printer", qty: 1, unitPrice: 436.3, amount: 436.3 },
    ],
  },
  {
    invoiceNo: "SI-2026-0003",
    customer: "Northstar Cafe Group",
    agent: "Daniel Lim",
    date: "06 Jun 2026",
    amount: "RM 3,420.00",
    paymentStatus: "Paid",
    currency: "MYR",
    outstandingAmount: "RM 0.00",
    details: [
      { description: "Branch Rollout Service", qty: 1, unitPrice: 1800, amount: 1800 },
      { description: "Kitchen Display Screen", qty: 2, unitPrice: 810, amount: 1620 },
    ],
  },
  {
    ...
    currency: "MYR",
    outstandingAmount: "RM 612.90",
    details: [
      { description: "Label Printer Repair", qty: 1, unitPrice: 312.9, amount: 312.9 },
      { description: "On-site Support Visit", qty: 1, unitPrice: 300, amount: 300 },
    ],
  },
  {
    invoiceNo: "SI-2026-0005",
    customer: "Greenfield Packaging",
    agent: "Marcus Lee",
    date: "04 Jun 2026",
    amount: "RM 2,105.45",
    paymentStatus: "Unpaid",
    currency: "MYR",
    outstandingAmount: "RM 2,105.45",
    details: [
      { description: "Warehouse Label Roll", qty: 5, unitPrice: 145.09, amount: 725.45 },
      { description: "Packaging Automation Setup", qty: 1, unitPrice: 1380, amount: 1380 },
    ],
  },
  {
    invoiceNo: "SI-2026-0006",
    customer: "Apex Office Systems",
    agent: "Wong Yi Thong",
    date: "03 Jun 2026",
    amount: "RM 498.00",
    paymentStatus: "Paid",
    ...
    outstandingAmount: "RM 0.00",
    details: [
      { description: "Cloud Backup Subscription", qty: 1, unitPrice: 498, amount: 498 },
    ],
  },
  {
    invoiceNo: "SI-2026-0007",
    customer: "Summit Maintenance",
    agent: "Amelia Tan",
    date: "02 Jun 2026",
    amount: "RM 1,780.20",
    paymentStatus: "Partial",
    currency: "MYR",
    outstandingAmount: "RM 890.10",
    details: [
      { description: "Field Service Deployment", qty: 1, unitPrice: 980.2, amount: 980.2 },
      { description: "Service Dashboard Training", qty: 1, unitPrice: 800, amount: 800 },
    ],
  },
  {
    invoiceNo: "SI-2026-0008",
    customer: "Evermark Services",
    agent: "Daniel Lim",
    date: "01 Jun 2026",
    amount: "RM 925.00",
    paymentStatus: "Unpaid",
    currency: "MYR",
    outstandingAmount: "RM 925.00",
    details: [
      { description: "Campaign Asset Retainer", qty: 1, unitPrice: 925, amount: 925 },
    ],
  },
];

const sortOptions: Array<{ key: SortKey; label: string; helper: string }> = [
  { key: "date", label: "Date", helper: "Newest sales invoices first" },
  { key: "amount", label: "Amount", helper: "Highest amount first" },
  ...
  { key: "paymentStatus", label: "Status", helper: "Attention first" },
];

const statusOptions: Array<{ key: StatusFilter; label: string; helper: string }> = [
  { key: "all", label: "All statuses", helper: "Show every sales invoice" },
...
function getSortValue(invoice: SalesInvoice, sortKey: SortKey) {
  if (sortKey === "amount") {
...
function resolveCustomerMeta(customer: string): CustomerMeta {
  return customerMetaMap[customer] ?? { color: "#615d59", Icon: Store };
}
...
function PaymentStatusPill({ status }: Readonly<{ status: string }>) {
  ...
  return (
    <span
      className={`inline-flex h-6 items-center gap-1.5 rounded-[6px] px-2 text-[13px] font-medium leading-5 ${pill}`}
    >
  ...
  const filteredInvoices = useMemo(() => {
    ...
    return salesInvoices.filter((invoice) => {
      const matchesSearch =
        ...
        invoice.agent.toLowerCase().includes(keyword);
    ...
    <div className="dashboard-shell min-h-screen bg-white md:flex">
      ...
      >
        <section className="flex min-h-screen flex-col">
            <header className="flex h-[var(--dashboard-header-h)] items-center justify-between border-b border-[#e9e6e2] px-[var(--dashboard-main-x)]">
              <div className="flex min-w-0 items-center gap-2">
                ...
                <h1 className="truncate text-[19px] font-semibold leading-7 tracking-[-0.01em] text-[#2c2c2b]">
                  Sales invoice
                </h1>
                <div className="group relative">
                  <button
                    type="button"
                    aria-label="About Sales Invoice"
                  ...
                  <div className="pointer-events-none absolute left-1/2 top-full z-30 mt-2 w-max max-w-[220px] -translate-x-1/2 rounded-[8px] bg-[#2c2c2b] px-3 py-2 text-[12px] leading-[1.45] text-white opacity-0 shadow-[0_4px_16px_rgba(15,15,15,0.18)] transition-opacity duration-150 group-hover:opacity-100">
                    Track and manage all AR sales invoices — customer, amount, payment status, and collection progress.
            ...
            <div className="flex h-[var(--dashboard-toolbar-h)] items-center justify-end gap-3 px-[var(--dashboard-main-x)]">
              <button
                ...
                disabled
                aria-disabled="true"
                className="inline-flex h-7 cursor-not-allowed items-center gap-1.5 rounded-[7px] bg-[#2783DE] px-2.5 text-[13px] font-medium leading-5 text-white opacity-55 shadow-[0_1px_1px_rgba(39,131,222,0.16)] outline-none"
              >
            ...
            <div className="flex h-[var(--dashboard-toolbar-h)] items-center gap-2 px-[var(--dashboard-main-x)]">
              <label className="relative w-[var(--dashboard-toolbar-search-w)] max-w-[42vw] shrink">
                <span className="sr-only">Search sales invoices</span>
                <Search
                  size={14}
                  strokeWidth={1.8}
                  aria-hidden="true"
                  ...
                  <tr>
                    {[
                      ...
                      },
                      {
                        label: "Status",
                        icon: Circle,
                        width: "w-[var(--salesinvoice-col-status)]",
                        key: "paymentStatus",
                      },
                      {
                        label: "Amount",
